...
function tile(label: string, value: string, unit: string, note: string, title?: string): React.JSX.Element {
  return (
    <div className="rz-tile" data-surface={`tile-${label}`}>
      <Metric label={label} value={value} unit={unit} title={title} />
...
export function SavingsView(props: SavingsViewProps): React.JSX.Element {
  ...
  return (
    <section aria-label="Savings" role="region">
      ...
      <div className="rz-tiles">
        {tile("Tickets", String(s.tickets), "tickets", "completed tickets this session")}
        {tile("Auto-sent", String(s.auto_sent), "replies", "replies sent without a human")}
        {tile("Deflection rate", deflectionPct, "%", "auto-sent share of completed tickets")}
        {tile("USD per ticket", s.usd_per_ticket.toFixed(4), "USD", "local estimate from the published price list — not a billing figure")}
        {tile("Hours saved", s.hours_saved.toFixed(1), "h", "assumption: 6 minutes per deflected ticket")}
        {tile("Source lookups", String(s.tickets), "lookups", "grounding attempts this session")}
      </div>
      ...
      <div data-result-region="cost-ledger">
        <div className="rz-view-header">
          <span className="eyebrow">Cost ledger</span>
          <HelpPopover regionId="cost-ledger" />
        </div>
        <table aria-label="spend by role" data-surface="cost-ledger">
          <tbody>
            <tr>
              <td>classifier</td>
